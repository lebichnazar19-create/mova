...
from . import екран, ескіз, креслення, малюнок, пристрій, розум
...
class ВМ:
    def __init__(self, без_пристрою=False):
        ...
        for ім_я in _ІМЕНА_МАЛЮНОК:
            if ім_я == "покажи":
                continue  # об'єднується з креслення.покажи() нижче, за активним режимом
        ...
        for ім_я in _ІМЕНА_КРЕСЛЕННЯ:
            if ім_я == "покажи":
                continue
            self._native[ім_я] = getattr(креслення, ім_я)
        for ім_я in _ІМЕНА_ЕСКІЗ:
            if ім_я == "довжина":
                continue  # об'єднується з мовним довжина() нижче, за арністю
            self._native[ім_я] = getattr(ескіз, ім_я)

        # "покажи": малюнок.py і креслення.py обидва описують «завершити й
        # показати малюнок» (див. докстрінг ядро/креслення.py) — реальний
        ...
        def _покажи_обʼєднане():
            if креслення._аркуш is not None:
                return креслення.покажи()
            if малюнок._полотно is not None:
                return малюнок.покажи()
