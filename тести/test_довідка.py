...
def test_є_кожне_ключове_слово_і_кожна_вбудована_дія():
    ...
    # і дії бібліотек (підключи("геометрія")), і нічого вигаданого, крім
    # вбудованих сталих (ПІ, вихід/вхід/вхід_підтяжка)
    from yadro import biblioteka
    from yadro.kompilyator import ВБУДОВАНІ_СТАЛІ
    бібліотечні = set(biblioteka.усі_імена())
    assert бібліотечні <= імена, бібліотечні - імена
    assert імена - set(КЛЮЧОВІ_СЛОВА) - ІМЕНА_NATIVE - бібліотечні == set(ВБУДОВАНІ_СТАЛІ)
...
def test_текст_довідки_згрупований_і_повний():
    ...
    assert текст.startswith("Мова ") and "\nДОВІДКА МОВИ\n" in текст   # перший рядок — версія/збірка/коміт
    for група, елементи in dovidka.ГРУПИ:
